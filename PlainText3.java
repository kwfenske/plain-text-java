/*
  ...
  --------------------------------
  PlainText3 is free software: you can redistribute it and/or modify it under
...
*/
  static void doConvertButton()
  {
    ...
    while (inputIndex < inputLength)
    {
      ...
      {
        /* Placeholder for customizing special requests. */

        changeCount ++;           // one more substitution has been made
        while (inputIndex < searchIndex) // for each UTF-32 character matched
        {
          resultBuffer.append("<" // Unicode character number with delimiters
      ...
      else if (dataEntry.action == ACTION_JAVA16) // Java backslash notation
      {
        ...
        while (inputIndex < searchIndex) // for all matched UTF-32 characters
      ...
      while ((inputText = inputFile.readLine()) != null)
      {
        ...
        else if (action == ACTION_REPLACE)
        {
          ...
          if (equalMatcher.find(inputIndex))
          {
            ...
            else if (keywordMatcher.find(inputIndex))
            {
              /* The right side of a "replace" can be a special word for a
              pre-defined action such as XML character references. */

              inputIndex = keywordMatcher.end();
              String special = keywordMatcher.group(1).toLowerCase();
